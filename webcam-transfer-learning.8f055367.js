...
google.pacman || function() {
    ...
    E.prototype.i = function(b) {
        var c = this.tilePos, d = l[this.dir], f = [
            c[0],
            c[1]
        ];
    ...
    E.prototype.o = function() {
        var b = this.pos[0] / 8, c = this.pos[1] / 8, d = [
            Math.round(b) * 8,
            Math.round(c) * 8
        ];
        ...
        if (this.id == g.playerCount && g.dotsRemaining < g.levels.elroyDotsLeftPart1 && this.mode == 2 && (!g.lostLifeOnThisLevel || g.actors[g.playerCount + 3].mode != 16)) {
            ...
            switch(this.id){
                ...
                case g.playerCount + 1:
                    ...
                    break;
                case g.playerCount + 2:
                    var d = g.actors[g.playerCount], f = [
                        b.tilePos[0],
                        b.tilePos[1]
                    ];
                    c = l[b.dir];
    ...
    E.prototype.d = function() {
        switch(this.currentSpeed){
            case 0:
                var b = this.id == 0 && (this.mode == 2 || this.mode == 1) ? g.cruiseElroySpeed : this.fullSpeed;
                break;
            case 1:
    ...
    g.startGameplay = function() {
        if (!g.timing) g.initializeTickTimer();
        g.score = [
            0,
            0
        ];
    ...
    g.changeGameplayMode = function(b) {
        g.gameplayMode = b;
        if (b != 13) for(var c = 0; c < g.playerCount; c++)g.actors[c].b();
        switch(b){
            case 0:
                g.playAmbientSound();
                break;
    ...
    g.handleGameplayModeTimer = function() {
        if (g.gameplayModeTime) {
            ...
            switch(g.gameplayMode){
                case 2:
                case 3:
                    for(var b = 0; b < g.playerCount; b++)g.actors[b].b();
                    break;
                case 10:
            ...
            if (g.gameplayModeTime <= 0) {
                ...
                switch(g.gameplayMode){
                    case 1:
                        ...
                        var c = e;
                        for(b = g.playerCount; b < g.playerCount; b++)if (g.actors[b] && (g.actors[b].mode == 4 || (g.actors[b].mode == 16 || g.actors[b].mode == 128) && !g.actors[b].eatenInThisFrightMode)) {
                            c = a;
                            break;
                        }
    ...
    g.handleForcePenLeaveTimer = function() {
        if (g.forcePenLeaveTime) {
            g.forcePenLeaveTime--;
            if (g.forcePenLeaveTime <= 0) g.resetForcePenLeaveTime();
        }
    };
    ...
    g.updateLoadingProgress = function(b) {
        var el = document.getElementById("logo-b");
        if (!el) return;
        b = Math.round(b * 200);
        el.style.width = b + "px";
    };
...
//# sourceMappingURL=webcam-transfer-learning.8f055367.js.map
